...
{
    double V_box = 1.0;
    if (!box_skew.empty() && is_skewed(box_skew)) {
        V_box = build_cell_geometry(box, box_skew, false, "rcpgenerator scale_diameters_nd").volume;
    } else {
        for (double L : box) {
            V_box *= L;
        }
...
{
    const bool skewed_periodic =
        is_skewed(box_skew) &&
        std::all_of(walls.begin(), walls.end(), [](std::int8_t wall) { return wall == 0; });

    if (skewed_periodic) {
        const auto geometry = build_cell_geometry(box, box_skew, false, "rcpgenerator get_pairs_nd_3");
        for (std::size_t i = 0; i < N; ++i) {
            pairs[i][0] = 0;
        }
        ...
        for (std::size_t i = 0; i < N; ++i) {
            for (std::size_t j = i + 1; j < N; ++j) {
                std::vector<double> delta(Ndim, 0.0);
                for (std::size_t d = 0; d < Ndim; ++d) {
                    delta[d] = x[j][d] - x[i][d];
                }
                const auto wrapped = minimum_image_delta(delta, geometry);
                const double d2 = vec_dot(wrapped, wrapped);
                const double r_c = (D[i] + D[j]) / 2 + t;
                if (d2 >= r_c * r_c) {
                    continue;
                }

                std::uint32_t& cnti = pairs[i][0];
                std::uint32_t& cntj = pairs[j][0];
                if (cnti >= neighborMax || cntj >= neighborMax) {
                ...
                ++cnti;
                pairs[i][cnti] = static_cast<std::uint32_t>(j);
                ++cntj;
    ...
    std::vector<std::size_t>& z)
{
    ...
    CellGeometry geometry;
    if (skewed_periodic) {
        geometry = build_cell_geometry(box, box_skew, false, "rcpgenerator get_forces_nd_3");
    }
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        for (std::uint32_t jdx = 1; jdx <= numNbr; ++jdx) {
            ...
            if (skewed_periodic) {
                std::vector<double> raw_delta(Ndim, 0.0);
                for (std::size_t d = 0; d < Ndim; ++d) {
                    raw_delta[d] = x[j][d] - x[i][d];
                }
                const auto wrapped = minimum_image_delta(raw_delta, geometry);
                for (std::size_t d = 0; d < Ndim; ++d) {
                    dx[d] = wrapped[d];
                    d2 += wrapped[d] * wrapped[d];
                }
            } else {
                for (std::size_t d = 0; d < Ndim; ++d) {
    ...
    const PackingRunOptions& options)
{
    ...
    if (box_skew.size() != Ndim) {
        throw std::runtime_error("Error: box_skew expects Ndim entries");
    }
    if (walls.empty()) {
        walls.assign(Ndim, 0);
    }
    ...
    CellGeometry geometry = build_cell_geometry(box, box_skew, true, "rcpgenerator run_packing");
    ...
    if (!options.fix_diameter) {
        auto scaled_pair = scale_diameters_nd(D, phi * phi_modifier, box, Ndim, fix_height, box_skew);
    ...
    double box_volume = geometry.skewed ? geometry.volume : std::accumulate(box.begin(), box.end(), 1.0, std::multiplies<double>());
    ...
    get_pairs_nd_3(N, Ndim, x, D, box, box_skew, walls, refresh, neighborMax, pairs);

    get_forces_nd_3(pairs, x, D, box, box_skew, walls, F, U, min_dist, max_min_dist, Lc, Fmean, mu, dkappa, z);
    ...
    for (std::size_t step = 1; step <= max_steps; ++step) {
        ...
        box_volume = geometry.skewed ? geometry.volume : std::accumulate(box.begin(), box.end(), 1.0, std::multiplies<double>());
