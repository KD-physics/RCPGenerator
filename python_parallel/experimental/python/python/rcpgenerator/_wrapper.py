...
def _is_skewed(box_skew: list[float]) -> bool:
    return any(abs(float(value)) > 1e-12 for value in box_skew)


def _build_cell_basis(box: list[float], box_skew: list[float]) -> list[list[float]]:
    ndim = len(box)
    if not box_skew:
        box_skew = [0.0] * ndim
    basis = [[0.0 for _ in range(ndim)] for _ in range(ndim)]
    for axis in range(ndim):
        theta = math.radians(float(box_skew[axis]))
        if axis == 0:
            basis[0][0] = float(box[0]) * math.cos(theta)
            if ndim > 1:
                basis[1][0] = float(box[0]) * math.sin(theta)
        else:
            basis[0][axis] = -float(box[axis]) * math.sin(theta)
            basis[axis][axis] = float(box[axis]) * math.cos(theta)
    return basis
...
class Packing:
    ...
    _INIT_KEYS = {"phi", "N", "Ndim", "box", "box_skew", "walls", "fix_height", "dist"}
    _PACK_KEYS = {"box", "box_skew", "walls", "neighbor_max", "seed", "fix_height"}
    ...
    def from_dict(cls, data: dict[str, Any]) -> "Packing":
        ...
        obj.box_skew = deepcopy(init_config.get("box_skew", pack_config.get("box_skew", [])))
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        default_box_skew = [0.0] * len(self.box) if self.box else []
        self.box_skew = deepcopy(kwargs.pop("box_skew", default_box_skew))
    ...
    def _collect_init_config(self) -> dict[str, Any]:
        return {
            ...
            "box_skew": deepcopy(self.box_skew),
    ...
    def _current_phi(self) -> float:
        ...
        if _is_skewed(list(self.box_skew)):
            basis = _build_cell_basis(list(self.box), list(self.box_skew))
            box_volume = abs(_matrix_determinant(basis))
        phi_modifier = 1.0
    ...
    def _apply_initializer_result(self, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.box = result["box"]
            self.box_skew = result.get("box_skew", [0.0] * len(self.box))
