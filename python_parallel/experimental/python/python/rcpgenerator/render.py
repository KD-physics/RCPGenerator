...
def _is_skewed(box_skew: Iterable[float]) -> bool:
    return any(abs(float(value)) > 1e-12 for value in box_skew)


def _basis_2d(box: list[float], box_skew: list[float]) -> tuple[np.ndarray, np.ndarray]:
    ax = np.deg2rad(float(box_skew[0]) if len(box_skew) > 0 else 0.0)
    ay = np.deg2rad(float(box_skew[1]) if len(box_skew) > 1 else 0.0)
    a = np.array([box[0] * np.cos(ax), box[0] * np.sin(ax)], dtype=float)
    b = np.array([-box[1] * np.sin(ay), box[1] * np.cos(ay)], dtype=float)
    return a, b


def _skew_vertices_2d(box: list[float], box_skew: list[float]) -> np.ndarray:
    a, b = _basis_2d(box, box_skew)
    return np.array(
        [
            [0.0, 0.0],
            a,
...
def _draw_2d_boundary(ax: plt.Axes, box: list[float], walls: list[int], box_skew: list[float] | None = None) -> None:
    kind = _container_kind(box, walls)
    if kind == "circle":
        ax.add_patch(Circle((box[0] / 2.0, box[0] / 2.0), box[0] / 2.0, fill=False, ec="black", lw=2.0))
    elif box_skew is not None and _is_skewed(box_skew):
        vertices = _skew_vertices_2d(box, box_skew)
        ax.plot(vertices[:, 0], vertices[:, 1], color="black", linewidth=2.0)
    else:
        x = [0, box[0], box[0], 0, 0]
        y = [0, 0, box[1], box[1], 0]
...
def _plot_2d(packing, palette_choice: int, figsize=(5, 5), dpi=120) -> plt.Figure:
    ...
    box_skew = list(getattr(packing, "box_skew", [0.0] * len(packing.box)))
    skewed = _is_skewed(box_skew)
    ...
    _draw_2d_boundary(ax, list(packing.box), list(packing.walls), box_skew=box_skew)
    if skewed:
        vertices = _skew_vertices_2d(list(packing.box), box_skew)
        x_min = min(float(vertices[:, 0].min()), float((positions[:, 0] - diameters / 2.0).min()))
        x_max = max(float(vertices[:, 0].max()), float((positions[:, 0] + diameters / 2.0).max()))
        y_min = min(float(vertices[:, 1].min()), float((positions[:, 1] - diameters / 2.0).min()))
        y_max = max(float(vertices[:, 1].max()), float((positions[:, 1] + diameters / 2.0).max()))
        pad_x = 0.05 * max(1e-9, x_max - x_min)
        pad_y = 0.05 * max(1e-9, y_max - y_min)
